...
export type Colors = ComponentProps<'div'> & ComponentProps<'strong'> & ComponentProps<'span'> & {
    darkColor: string,
    lightColor: string,
    title: string,
    qtd?: number
...
export const TableTask = ({ darkColor, lightColor, title, qtd }: Colors) => {
  return (
    <article className="bg-[#1E293B1A] rounded-3xl">
      <div className={`bg-[${lightColor}] flex items-center justify-between py-1 pl-2 pr-3 rounded-full`}>
        <div className="flex items-center gap-2.5">
          <strong className={`bg-[${darkColor}] text-white text-xs pt-1.5 pb-2.5 px-3 rounded-3xl font-semibold`}>{qtd}</strong>
